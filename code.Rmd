...
keep <- apply(my_data.fpm, 1, function(row) all(row >2))
...
#plotcounts
par(mfrow=c(2,3))
plotCounts(dds, gene="ENSG00000237223|SULT1C2P1", intgroup = "healthy")

```

```{r}
# Paste the two factors together
#groups <- factor(c(rep(1, 64), rep(2, 20)),
#                                   labels = c("Sick", "Healthy"))
#groups <- as.factor(paste(annotation$Cell, phenodata$characteristics_ch1.5, phenodata$characteristics_ch1.6, sep= "_"))

# healthy group as control
#groups <- relevel(groups, "treatment")

# Create a new annotation data frame
#annotation <- data.frame(group = groups, row.names = names(my_data))

# Simplified design
#design <- model.matrix(~ groups)

#summary(results(dds, contrast = c("groups", "lesion_argonist", "healthy_argonist"),
#alpha = 0.05))
